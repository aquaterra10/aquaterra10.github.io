...
function applyLang(){const t=TRANSLATIONS[lang]||TRANSLATIONS.fr;document.documentElement.lang=lang;$$('[data-i18n]').forEach(el=>{const k=el.dataset.i18n;if(t&&t[k])el.textContent=t[k]});$$('[data-i18n-html]').forEach(el=>{const k=el.dataset.i18nHtml;if(t&&t[k])el.innerHTML=t[k]});$$('[data-i18n-placeholder]').forEach(el=>{const k=el.dataset.i18nPlaceholder;if(t&&t[k])el.placeholder=t[k]});$$('.language-option').forEach(b=>{const a=b.dataset.lang===lang;b.classList.toggle('active',a);b.setAttribute('aria-pressed',a)});renderSelects();if($('#demoStage'))renderDemo(currentDemo)}
...
try{initDemos()}catch(e){console.error('initDemos',e)}
try{estimate()}catch(e){console.error('estimate',e)}
try{reveal()}catch(e){console.error('reveal',e);$$('.reveal').forEach(x=>x.classList.add('visible'))}
try{network()}catch(e){console.error('network',e)}
